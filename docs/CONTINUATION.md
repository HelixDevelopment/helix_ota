...
**Revision:** 18
**Last modified:** 2026-07-09T19:26:31Z
...
F. **Expand §11.4.170 screen×state matrix.** Stream **R** DONE (`df2784ec`): dashboard now proves **9** screens ×{light,dark} (added Deployments/Fleet/Groups/Overview; 45→81 host-render pass; found+fixed a real layout-oracle substring bug). Stream **U** in flight adding empty+error STATE variations for 2 core dashboard screens (screen×STATE×theme). ota-manager still owes its matrix beyond LoginPage (unrouted pages + 85 router-cluster errors block it; bundle after C).
...
L. **Memory + Fuzz gaps — Stream W in flight:** add a heap/goroutine-growth assertion for core API handlers (only the embed handler has a leak check) + a native Go `Fuzz*` target (currently ZERO fuzz targets in server/).
M2. **Postgres integration suite unexecuted** — the `-tags integration` pgx path TYPE-CHECKS clean but was never run (no live PG/containers booted). Boot Postgres via the containers submodule (§11.4.161 rootless) + capture real evidence.
N. **Benchmark regression baseline** — 7 real benchmarks exist but nothing gates regression; add a baseline + comparison.
O. **Security follow-ups** — no security-response-headers set anywhere (product decision: add headers + test); a cross-secret token-forgery test (V corrected: scheme is fixed HMAC-SHA256, not multi-alg JWT, so `alg=none` doesn't apply — positive finding); SQL/NoSQL-injection probe against the real pgx path inside `server/`.
P. **Aborted security-suite run** — `tests/security/RUN_EVIDENCE.txt` last run ABORTED on a port conflict (unlike its clean-PASS siblings); re-run resolving the conflict + capture fresh evidence.
Q. **DDoS default posture** — the rate-limit cap is disabled by default (`HELIX_MAX_INFLIGHT=0`); decide the shipped default.
