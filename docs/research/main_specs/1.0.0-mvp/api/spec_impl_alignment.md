...
| Created | 2026-06-08 |
| Status | active (decision memo — additive + group breaking WIDENs landed; row-4 per-device telemetry **filters** + row-7 group/members **pagination** RESOLVED 2026-06-10; row-4 richer numeric fields `duration_ms`/`bytes_transferred` **RESOLVED — now ingested end-to-end 2026-06-10**) |
| Landed (2026-06-10, telemetry numeric fields) | **Row 4 richer fields RESOLVED** — per-item `duration_ms` + `bytes_transferred` are now ingested end-to-end (`71be1cd`, `feat(telemetry): per-event duration_ms + bytes_transferred end-to-end (now ingested)`). The Tier-1 device emulator is now a real event source emitting `duration_ms` on every lifecycle event + `bytes_transferred` on download events; threaded additively emulator → server ingest (`handlers_client.go`) → store (memory + pgx, nullable cols) → read VIEW (`handlers_telemetry.go`, `omitempty`). This closes the previously-DEFERRED "blocked on UNVERIFIED ingest (§9)" qualifier — the §9 register is updated to FACT below. Anti-bluff evidence: `server/internal/deviceemu/telemetry_fields_test.go:TestEmulatorTelemetryFieldsEndToEnd` asserts the EXACT values flow emulator→server→read-API over real HTTP; `TestEmulatorTelemetryFieldsIngestMutation` is the §1.1 paired mutation; real-Postgres parity proven via `server/internal/store/postgres_integration_test.go:TestPostgresRepositoryContract_Integration` (podman-booted PG, 0 skips). Evidence dir `docs/qa/20260610T140407Z-telemetry-fields/`. |
| Landed (2026-06-10) | **Row 4 filters** — per-device telemetry now accepts the spec filter params (`50ef5c6`, `feat(api): per-device telemetry filters + group/members pagination`; OpenAPI synced + redocly-clean). **Row 7 pagination** — group + members list now paginated. Both previously-DEFERRED WIDEN bits are RESOLVED. Dashboard API client updated in lockstep (`b0b8ee2`). |
| Operator ruling (2026-06-08) | **WIDEN impl to the fuller spec** for the remaining breaking divergences. Executing in sub-batches. **Landed:** group row 6 id-key `id`→`group_id` + row 8 member-add → batch `{device_ids}` → 200 `{added, already_member, not_found}` (device-registration-aware); **row 1 audit `actor` → object `{user_id, subject}`**; **row 4 structural** — per-device telemetry `events`→`items` + `next_cursor`, **newest-first**, `?limit`/`?cursor` pagination (handler-side over bounded history). **`GET /groups/{id}/members` → `items[]` of `{device_id, added_at}`** (store gained an `added_at` membership column + `ListGroupMembersDetailed` on memory + pgx, real-DB parity). Remaining: row 4 DEFERRED bits (richer per-item fields `duration_ms`/`bytes_transferred` — UNVERIFIED ingest; `?event_type`/`?since`/`?until`/`?deployment_id` filters), row 7 (group + members list pagination — deferred, groups bounded). |
...
| 4 | per-device telemetry: pagination + newest-first + filters + richer fields | `?limit/?cursor/?event_type/?since/?until/?deployment_id`, newest-first, `items[]` with `id/success/duration_ms/bytes_transferred` | newest-first `items[]` + `?limit/?cursor` (R7, 2a48ab5) + filter params **RESOLVED 2026-06-10 (`50ef5c6`)**; richer numeric fields `duration_ms`/`bytes_transferred` **RESOLVED — ingested end-to-end 2026-06-10 (`71be1cd`)** (`handlers_telemetry.go:handleDeviceTelemetry` / `TelemetryEventView` now exposes them with `omitempty`) | **WIDEN-impl — FULLY RESOLVED 2026-06-10** | Pagination + newest-first shipped R7; the `?event_type`/`?since`/`?until`/`?deployment_id` filters landed `50ef5c6`; `duration_ms`/`bytes_transferred` now ingested end-to-end (`71be1cd`) — the Tier-1 emulator emits them, threaded additively emulator→server ingest→store (memory+pgx)→read VIEW. No longer a standing divergence. | **PROVEN:** `server/internal/deviceemu/telemetry_fields_test.go:TestEmulatorTelemetryFieldsEndToEnd` (exact-value flow over real HTTP) + `TestEmulatorTelemetryFieldsIngestMutation` (§1.1 pair); real-PG parity `server/internal/store/postgres_integration_test.go:TestPostgresRepositoryContract_Integration`; evidence `docs/qa/20260610T140407Z-telemetry-fields/` |
...
| 7 | `GET /groups` + `GET /groups/{id}/members` pagination | paginated `{items, next_cursor}` (+ `?name=` prefix on groups) | **RESOLVED 2026-06-10 (`50ef5c6`)** — group + members list now paginated (`handlers_group.go:handleListGroups`/`handleListGroupMembers`) | **WIDEN-impl — LANDED 2026-06-10** (was TRIM-spec/defer) | Previously deferred (groups bounded); widened to paginated `{items, next_cursor}` this session alongside the telemetry-filter work, with the dashboard client updated in lockstep (`b0b8ee2`). No longer a standing divergence. | `handleListGroups`/`handleListGroupMembers` param tests + `store.ListGroups`/members filter/cursor store test + integration; bank: group-CRUD / group-membership Challenges re-assert the paginated shape |
...
3. **Per-device telemetry pagination + newest-first + filters + rich fields (row 4) — FULLY LANDED 2026-06-10.**
   Prevented the unbounded-history payload cliff (pagination + newest-first + filters, `50ef5c6`).
   The `duration_ms`/`bytes_transferred` fields — previously deferred until the ingest was confirmed
   to supply them — now ingest end-to-end (`71be1cd`): the Tier-1 emulator is the real event source.
   The breaking key change (`events`→`items`) landed with the dashboard client updated in lockstep.
   Row 4 is no longer a standing divergence.
...
- **UNVERIFIED — effort/risk sizes.** The S/M/L sizes and LOW/MED risk labels are estimates from
  ...
  claimed.
- **RESOLVED — telemetry rich fields availability (row 4) (was UNVERIFIED; now FACT 2026-06-10, `71be1cd`).**
  The earlier qualifier ("whether `store.TelemetryRecord` / the `ota-protocol` telemetry event
  already carries `duration_ms` / `bytes_transferred`") is no longer open: the Tier-1 device
  emulator was made a real event source that emits `duration_ms` on every lifecycle event +
  `bytes_transferred` on download events, threaded additively emulator → server ingest
  (`handlers_client.go`) → store (memory + pgx, nullable columns) → read VIEW
  (`handlers_telemetry.go` `TelemetryEventView`, `omitempty`). The "always-zero columns (§11.4-bluff)"
  risk is closed because the values are ingested end-to-end and asserted by exact-value tests —
  NOT a zero default. Proof: `server/internal/deviceemu/telemetry_fields_test.go:TestEmulatorTelemetryFieldsEndToEnd`
  + `TestEmulatorTelemetryFieldsIngestMutation` (§1.1 pair) + real-PG parity
  `server/internal/store/postgres_integration_test.go:TestPostgresRepositoryContract_Integration`;
  evidence `docs/qa/20260610T140407Z-telemetry-fields/`.
