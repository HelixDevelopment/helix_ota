...
| Created | 2026-06-08 |
| Status | active (decision memo — additive WIDENs landed; breaking ones pending operator ruling) |
| Landed (2026-06-08, operator-approved additive WIDENs, commit 028e656) | **Row 2 (audit query filters)** — `?since`/`?until` RFC3339 bounds implemented on `GET /audit` (memory + pgx, real-DB parity). **Row 5 core (telemetry overview)** — `failure_rate = failure/(success+failure)` + `by_state` (device-state map) added to `GET /telemetry/overview`; both additive (new optional params / new JSON fields), wire-compatible, no contract break. Remaining rows (1 audit actor object, 4 per-device telemetry pagination/`events`→`items`, 8 batch member-add) are BREAKING wire changes still gated behind an operator WIDEN/TRIM ruling. |
