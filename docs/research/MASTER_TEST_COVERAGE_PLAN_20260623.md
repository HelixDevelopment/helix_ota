...
| stress/chaos | ota-* bricks have it; **HTTP load vs live DONE** — p50/p90/p99 **3.69/8.54/14.32ms @ 5,540 req/s, 166K reqs, zero 5xx, no saturation** (docs/qa/20260623-http-load-live/) | + chaos fault-injection | **HTTP load DONE**; server-layer chaos pending |
| benchmark | 7 exist, no baseline | benchstat registry | gap |
| Android instrumentation/ui | **JaCoCo MEASURED** (wired both bricks): ota-update-engine-bridge **LINE 100%** (113/113, 27 tests), ota-android-agent **LINE 91.18%** (362/397, 47 tests; json-codec branch 55.6% = honest gap); docs/qa/20260623-android-jacoco/ | branch gaps + on-device A/B | **JVM coverage DONE**; json-branch + on-device pending |
