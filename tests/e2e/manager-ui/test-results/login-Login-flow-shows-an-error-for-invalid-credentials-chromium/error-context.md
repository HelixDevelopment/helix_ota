...
- Name: login.spec.ts >> Login flow >> shows an error for invalid credentials
- Location: e2e/login.spec.ts:60:7
...
Locator: locator('text=/invalid|error|failed|incorrect/i')
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('text=/invalid|error|failed|incorrect/i')

```

```yaml
- text: OTA Manager Sign in to access the OTA management dashboard. Email
...
```ts
  1   | // tests/e2e/manager-ui/e2e/login.spec.ts
  2   | //
