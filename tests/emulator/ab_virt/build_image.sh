...
SYSEOF
    ...
    mkdir -p /work/rootfs-overlay/boot
    cat > /work/rootfs-overlay/etc/fstab <<FSTAB
# /etc/fstab for A/B virt guest (PWU-AB-2) — mounts the FAT boot partition at /boot
# so fw_setenv can access uboot.env on p1. Kernel mounts root from bootargs, not fstab.
/dev/vda1  /boot  vfat  defaults  0  0
FSTAB

    cat > /work/rootfs-overlay/etc/fw_env.config <<FWEOF
# U-Boot environment via FAT file (CONFIG_ENV_IS_IN_FAT, PWU-AB-2)
# Both U-Boot and fw_setenv/fw_printenv access the same file.
# U-Boot: CONFIG_ENV_IS_IN_FAT -> virtio 0:1 uboot.env
# Linux:  /boot/uboot.env (p1 mounted at /boot via fstab, offset=0 start-of-file)
# The env binary is generated by assemble_ab_disk.sh: mkenvimage -s 0x4000
# Verified 2026-06-19: CONFIG_ENV_SIZE=0x4000 matches the -s argument.
# Sources: https://docs.u-boot.org/en/latest/develop/environment.html
...
FWEOF
    ...
    cp /work/rootfs-overlay/etc/rauc/dev.cert.pem /work/out/rauc-keys/dev.cert.pem
    ...
    # ---- Build RAUC update bundle inside the SAME aarch64 container -----------
    # The container is a REAL Linux environment where rauc can run natively with
    # full kernel support (loop devices, mounts, etc.), unlike the macOS host.
    # This is the recommended path per research §Q5 Option A.
    ...
    # Render manifest from template using sed (no .in file needed in container)
...
RAUCM
    ...
    ls -la /work/out/bundle-src/
    echo "RAUC: building signed dm-verity bundle (manifest: /work/out/bundle-src/manifest.raucm)"
    rauc --cert /work/out/rauc-keys/dev.cert.pem \
         --key /work/out/rauc-keys/dev.key.pem \
         bundle /work/out/bundle-src/ /work/out/images/update.raucb 2>&1
    echo "RAUC: bundle result:"
    ls -la /work/out/images/update.raucb 2>/dev/null || echo "RAUC: WARNING bundle not produced"
    # ---- end RAUC bundle build --------------------------------------------------
...
podman cp "${BUILD_CTR}:/work/out/rauc-keys/dev.cert.pem" "${OUT}/rauc-keys/dev.cert.pem" >>"${OUT}/build.log" 2>&1 || true
chmod 600 "${OUT}/rauc-keys/dev.key.pem" 2>/dev/null || true
chmod 644 "${OUT}/rauc-keys/dev.cert.pem" 2>/dev/null || true
podman rm -f "$BUILD_CTR" >/dev/null 2>&1 || true
