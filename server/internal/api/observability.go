...
func NewMetrics(reg prometheus.Registerer) *Metrics {
	...
	g, _ := reg.(prometheus.Gatherer) // DefaultRegisterer is also a Gatherer
	m := &Metrics{
		gatherer: g,
...
func (m *Metrics) Middleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Use the route pattern (c.FullPath) as the path label — it has low,
		// bounded cardinality. When unknown (e.g. a 404 that matched no route),
...
// ctxLogger is the Gin context key for the per-request slog.Logger.
...
// Handlers retrieve it via LoggerFrom.
...
// NewJSONLogger creates a slog.JSONHandler writing to os.Stdout at the given
